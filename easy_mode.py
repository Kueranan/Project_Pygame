...
def easy_mode():
    ...
    font_size = 20
    choice_size = (250, 80)
    oChoice_1 = Choice(screen, (WIDTH*2//7, HEIGHT*4.125//5), 'images/choice.png', 'images/choice_down.png', font_size, size = choice_size)
    oChoice_2 = Choice(screen, (WIDTH*5//7, HEIGHT*4.125//5), 'images/choice.png', 'images/choice_down.png', font_size, size = choice_size)
    
    #timer
    rew = c_reward()
    timmer = c_timer(screen,"easy")
    ...
    while run:
        ...
        if  0.5 <= rew.grade <= 4 and i <= 18 and timmer.timer_ingame >= 0:
            ...
            i = 0
            list_choice.clear()
            Q_A_check = qusetion_eqution_answer(Need_total_Qusetion=7, choice=7)
            check_Q, check_c = Q_A_check.check_qusetion()
            list_choice += [check_Q[i][0], check_c[i]]
            ran_list = random.sample(list_choice, k=2)
            time.sleep(0.3)
